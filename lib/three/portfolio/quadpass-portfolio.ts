...
interface QuadPassPortfolioParams{
...
export class QuadPassPortfolio{
  renderer: THREE.WebGLRenderer;
  pixel_density: number;
  buffer_param: THREE.RenderTargetOptions;
  shader: {
  ...
  constructor(params: QuadPassPortfolioParams){
    ...
    this.buffer = new THREE.WebGLRenderTarget(
      this.width * this.pixel_density,
      this.height * this.pixel_density,
      this.buffer_param
    );
