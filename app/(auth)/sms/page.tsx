  ...
  return(
    <div className="mt-10 max-w-full lg:max-w-4/5 xl:max-w-3/5 mx-auto">
      ...
      flex flex-col justify-center items-center gap-2 text-center">
        ...
        form-title">Log In with SMS</h1>
        <h1 className="font-weight-basic text-2xl form-h2">휴대폰을 통한 인증이 필요해요</h1>
      </div>

      <form action={action} className="flex flex-col gap-3 mb-3.5">
        {state.token ? <FormInput name="token" required type="number" placeholder="Verification Code"
        min={100000} max={999999}
        errors={state.error?.formErrors} /> 
        : <FormInput name="phone" required type="text" placeholder="Phone Number" errors={state.error?.formErrors} />}
        
        <FormBtn text={state.token ? "Verify" : "Send"} />
      </form>
    </div>
  );
}