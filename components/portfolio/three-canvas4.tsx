...
import { SceneManager4 } from "@/lib/three/portfolio/scene-manager4";
...
export default function ThreeCanvas4({work}:ThreeCanvasProps){
  const refs = useThree4Refs();
  const {containerRef} = refs;
  const sceneManagerRef = useRef<SceneManager4 | null>(null);
  ...
  useEffect(()=>{
    ...
    //3.새 인스턴스 생성
    const sceneManager = new SceneManager4(refs);
